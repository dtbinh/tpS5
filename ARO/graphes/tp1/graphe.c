...
/* OpÃ©rations d'ajout pour les tableaux */
...
/* Remet Ã  zÃ©ro un graphe, sans le dÃ©sallouer */

static void auxResetGraphe(tGraphe graphe) {
  tNumeroSommet i,j;  
  /* matrice d'adjacence mise Ã  zÃ©ro */
  graphe->nbSommets = 0;
  for(i=0;i<MAX_SOMMETS;i++) 
    for(j=0;j<MAX_SOMMETS;j++) 
      graphe->matriceAdjacence[i][j]=0;

  /* table des successeurs, prÃ©dÃ©cesseurs et voisins mise Ã  zÃ©ro */
  for(i=0;i<MAX_SOMMETS;i++) {
...
tGraphe grapheAlloue() {
  ...
  if (graphe==NULL) 
    halt("Erreur d'allocation mÃ©moire dans grapheAlloue\n");
...
void grapheAleatoire(tGraphe graphe, 
  ...
  if ( (probaArc<0.0) || (probaArc>1.0) )
    halt("La probabilitÃ© doit Ãªtre entre 0.0 et 1.0");
...
static int lireToken(FILE* fic, char token[LONG_MAX_TOKEN]) {
  ...
  while ( !termine && (c != EOF)) {
    ...
    // si c'est un blanc ou un car spÃ©cial
  ...
  token[nbCarToken] = '\0';
  // si aucun token n'a Ã©tÃ© lu, on renvoie -1
...
/* idem lire token mais arrÃªte le programme si aucun token ne peut
   Ãªtre lu */
...
int grapheChargeFichier(tGraphe graphe, char *fichier) {
  ...
  /* dÃ©termination du type de l'arbre */
  lireTokenGaranti(fic, token);
  ...
  lireTokenGaranti(fic, token);
  while (strcmp(token,";")) {
    /* token est le sommet de dÃ©part */
    strcpy(orig, token);
...
void grapheAjouteArc(tGraphe graphe, tArc arc) {
  ...
  /* On teste si l'arc existe dÃ©jÃ  ou pas */
  if (grapheExisteArcEntre(graphe, orig, dest)) {
    ...
    halt("Un arc existe dÃ©jÃ  entre les sommets %s (numÃ©ro %d) "
	 "et %s (numÃ©ro %d)\n",
  ...
  if (grapheEstOriente(graphe)) {
    ...
    /* ajout dans les prÃ©dÃ©cesseurs */
    tableauEntiersAjoute(& (graphe->tablePredecesseurs[dest]), orig);
  } 

  /* MÃªme si l'arbre est orientÃ©, on remplit la table des voisins.
     Toutefois, si l'arc est orientÃ©, on vÃ©rifie que l'arc
     inverse n'existe pas dÃ©jÃ , car dans ce cas les voisins sont
     dÃ©jÃ  placÃ©s. */
  
  if ( ! ( grapheEstOriente(graphe) && 
	   grapheExisteArcEntre(graphe,dest,orig) ) ) {
      tableauEntiersAjoute(& (graphe->tableVoisins[orig]), dest);
      /* si l'arc va du sommet vers lui mÃªme, on n'ajoute pas de voisin */
      if (orig!=dest)
	tableauEntiersAjoute(& (graphe->tableVoisins[dest]), orig);
    }
  /* Mise Ã  jour de la table des arcs */
...
void grapheAffiche(tGraphe graphe) {
  ...
  /* Affiche les voisins, successeurs, prÃ©decesseurs */
  if (grapheEstOriente(graphe)) {
    ...
    /* affichage des prÃ©dÃ©cesseurs */
    printf("\nListe des prÃ©dÃ©cesseurs :\n");
    for(i=0; i < grapheNbSommets(graphe);i++) {
      grapheRecupNomSommet(graphe, i, nom);
      printf("%s : ", nom);
...
/* VÃ©rifie que le numÃ©ro de sommet est correct */
...
int grapheNbSuccesseursSommet(tGraphe graphe, tNumeroSommet sommet){
  ...
  if (!grapheEstOriente(graphe)) 
    halt("Fonction rÃ©servÃ©e aux graphes orientÃ©s");
...
tNumeroSommet grapheSuccesseurSommetNumero(tGraphe graphe, 
  ...
  if ( (numero<0) || (numero>=nbSuccesseurs) ) {
    halt("Numero de successeur incorrect (%d) pour le sommet numÃ©ro %d", 
	 numero, sommet);
  }
...
int grapheNbVoisinsSommet(tGraphe graphe, tNumeroSommet sommet) {
  auxVerifieNumeroSommet(graphe, sommet);
  /*if (grapheEstOriente(graphe)) 
    halt("Fonction rÃ©servÃ©e aux graphes non orientÃ©s");*/
...
tArc grapheRecupArcNumero(tGraphe graphe, int numero) {
  if ( (numero<0) || (numero>grapheNbArcs(graphe)) )
    halt("NumÃ©ro d'arc incorrect : %d\n", numero);
...
tFileSommets fileSommetsAlloue() {
  ...
  if (file==NULL) 
    halt("Erreur d'allocation mÃ©moire dans fileSommetsAlloue\n");
  file -> debut = 0;
  file -> fin = 0;
  return file;
}
...
tNumeroSommet pileSommetsTete(tPileSommets pile) {
  if (pileSommetsEstVide(pile))
    halt("Ne peut renvoyer la tÃªte d'une pile vide");
...
tNumeroSommet pileSommetsDepile(tPileSommets pile) {
  tNumeroSommet res;
  if (pileSommetsEstVide(pile))
    halt("Ne peut dÃ©piler d'une pile vide");
